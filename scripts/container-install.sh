...
# 1. Throwaway build venv: install the hatchling build backend and its full
#    transitive build deps so the okp_mcp wheel can be built without network
#    access. This venv is never copied to the runtime stage.
#    Both build requirements files are passed so pip resolves the full
#    transitive tree in one transaction (required by --require-hashes).
