...
#   APP_VENV                 — path to the application venv
...
if [ -f /cachi2/cachi2.env ]; then
    ...
    "${APP_VENV}/bin/pip" install --no-cache-dir "${PIP_BINARY_FLAG}" --require-hashes \
    ...
    "${APP_VENV}/bin/pip" install --no-cache-dir --no-deps --no-index \
        --find-links "${HOME}/wheels" okp_mcp

    # 5. Smoke-test: verify the package is importable before finishing the layer.
    "${APP_VENV}/bin/python" -c "import okp_mcp"
else
    # --- Local / non-hermetic build ---
    # Network available. Install from the .konflux/requirements.txt manifest
    # (hash-pinned, generated from pdm.lock by scripts/konflux_requirements.sh).
    # No pdm needed in the container — pip handles the install directly.

    python3 -m venv "${APP_VENV}"

    # Install hash-pinned runtime dependencies.
    "${APP_VENV}/bin/pip" install --no-cache-dir "${PIP_BINARY_FLAG}" --require-hashes \
        -r .konflux/requirements.txt

    # Install the project itself (no editable install in containers).
    "${APP_VENV}/bin/pip" install --no-cache-dir --no-deps .
fi
